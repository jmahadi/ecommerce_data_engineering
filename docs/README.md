...
Setup Steps
Clone and start the containers:
bash
git clone <your-repo>
...
# NOTE: ngrok URL changes every restart, so you'll need to update 
# the connection strings in all your Looker Studio data sources each time
# I exported the dashboards as PDFs (saved in dashboards/ folder) as backup
# since the live connection isn't permanent
...
Staging Load - Raw CSV data gets loaded into staging tables with basic validation
Warehouse Transform - Data gets cleaned and transformed into star schema with proper joins
Analytics Aggregation - Business metrics get calculated and stored for fast dashboard queries
Data Quality
...
Local Publisher Educational Books: 88.51% margin
Local Publisher Fiction Books: 59.9% margin
Top performers actually span multiple categories, with Electronics having some of the highest-margin individual products despite lower category average
What are the top customer segments by lifetime value?
Regular customers: $7.75M total (56.9% of base, avg 4.81 orders)
Premium customers: $3.82M total (28% of base, avg 4.69 orders)
Budget customers: $2.05M total (15.1% of base, avg 4.88 orders)
...
Slight dip in July 2025 (artifact of data generation timing)
Which marketing campaigns generate the best ROI?
"Re-contextualized cohesive focus" campaign generated the highest revenue at ~$2M
"Horizontal multimedia collaboration" and "Polarized system-worthy access" follow at ~$1.8M each
Top 3 campaigns significantly outperform others in the $500K-800K range
What's the average time between customer acquisition and first purchase?
67.8 days average
Most customers (1.4K) purchase "After Month" rather than immediately
Same day converters are rare but valuable
...
Data Type Casting
PostgreSQL was pretty strict about data types when loading from staging to warehouse. Had to add explicit casting (::date, ::numeric) in several places to make the transformations work properly.
...
But for an assignment this demonstrates the core concepts pretty well.
...
This was a pretty fun project - got to work with the full data engineering stack and actually see some interesting insights in the data. The fake data generator did a good job creating realistic patterns that make the analysis meaningful. The resource constraints taught me about balancing local development environments and making pragmatic technical decisions too.
