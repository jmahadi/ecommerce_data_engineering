...
I went with a pretty standard data warehouse approach with three schemas:
...
- `monthly_trends` - for seasonal analysis
- `customer_acquisition` - time to first purchase analysis
...
1. **Staging Load** - Raw CSV data gets loaded into staging tables
2. **Warehouse Transform** - Data gets cleaned and transformed into star schema
3. **Analytics Aggregation** - Business metrics get calculated and stored
...
- Basic business logic validation (like prices > 0)
...
- Profit margin vs sales volume analysis
...
1. **Which products have the highest profit margins?** 
   - Answer: Books and Health & Beauty products consistently show 58-60% margins
   - Electronics has lower margins (~45%) but highest volume
...
3. **How do seasonal trends affect sales performance?**
   ...
   - Slight dip in July 2025 (might be data generation artifact)

4. **Which marketing campaigns generate the best ROI?**
   - Campaign attribution shows revenue ranges from $50K to $800K per campaign
...
But for an assignment this approach works pretty well and demonstrates the core concepts.
...
This was a pretty fun project - got to work with the full data engineering stack and actually see some interesting insights in the data. The fake data generator did a good job creating realistic patterns that make the analysis meaningful. The resource constraints taught me about balancing local development environments too.